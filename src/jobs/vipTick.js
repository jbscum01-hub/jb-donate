...
");
    ...
    for (const v of warn) {
      ...
      await dmUser(v.user_id, `⏳ VIP (${v.vip_code}) ของคุณจะหมดอายุภายใน 24 ชั่วโมง (หมดอายุ: ${v.expire_at})`);
    ...
    for (const v of expired) {
      ...
      await dmUser(v.user_id, `🧾 VIP (${v.vip_code}) หมดอายุแล้ว ระบบได้ถอดยศ VIP อัตโนมัติค่ะ`);
    }
  } finally {
    process.exit(0);
  }
