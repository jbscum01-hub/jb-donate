...
function parseOrderNo(customId) {
  ...
  const legacy = String(customId || "").match(/^staff_close:(.+)$/);
...
async function refreshVehicleCard(client, plate, kind) {
  ...
  const plateLogCh = await client.channels.fetch(IDS.VEHICLE_PLATE_LOG_CHANNEL_ID).catch(() => null);
  if (!plateLogCh) return;
...
export async function closeOrder(interaction) {
  try {
    ...
    if (archiveCh) {
      if (!files.length) {
        ...
        for (let i = 0; i < files.length; i += chunkSize) {
          ...
          await archiveCh.send({ content: header, files: chunk }).catch(() => {});
        }
      }
    }
    ...
    await safeReply(interaction, { content: "✅ ปิดออเดอร์แล้ว กำลังลบห้อง…", ephemeral: true });
    await ticketCh.delete("Ticket success closed").catch(() => {});
  } catch (error) {
    console.error("closeOrder error", error);
    return safeReply(interaction, { content: `❌ CLOSE ไม่สำเร็จ: ${error?.message || String(error)}`, ephemeral: true }).catch(() => {});
  }
}
